...
def Buscar(raiz_apps=[]):
    #Pacotes Usados
    import os


    def AppsConteudo(pasta_raiz):
        """
        -> Conteudo das pastas de Aplicativos
        :param apps: Local da pasta raiz onde estar os aplicativos
        ...
        try:
            ...
            for c in range(0,len(pasta_raiz)):
                ...
                aplicativos_diretoria[pasta_raiz[c]] = os.listdir(f"apps/{pasta_raiz[c]}")
    ...
    def Busca_shinc(pasta_raiz,nome):
        ...
        :param aplicativos: Conteudo que foi retornado de  AppsConteudo
        :param apps: Local da pasta raiz onde estar os aplicativos
                    ...
                    try:
                        for extensao in range(0,int(len(arq_extensao[nomes[c]][intes])/2)):
                            ...
                            if arq_extensao[nomes[c]][intes][extensao+1] == "shinc":
                                args[nomes[c]] = f'{arq_extensao[nomes[c]][intes][extensao]}.{arq_extensao[nomes[c]][intes][extensao+1]}'
                    except:
